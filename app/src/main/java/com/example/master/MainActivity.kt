...
fun TapToPhoneScreen(activity: ComponentActivity, modifier: Modifier = Modifier) {
    ...
    fun refreshEnrollment() {
        if (configured) {
            runCatching { TapToPhoneManager.mposUi.tapToPhone.isDeviceEnrolled() }
                ...
                .onFailure { message = "Unable to initialize Tap to Phone. Check merchant setup." }
        }
    }
    ...
    fun enroll() {
        ...
        }.onFailure { busy = false; message = "Unable to open device enrollment." }
    }
    ...
    fun launchPayment(intent: Intent) {
        runCatching { busy = true; paymentLauncher.launch(intent) }
            .onFailure { busy = false; message = "Unable to open payment. No approval received." }
    }
    ...
    Column(modifier.verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (!configured) {
            ...
            OutlinedTextField(currency, { currency = it }, enabled = !busy, label = { Text("Currency (e.g. EUR)") }, singleLine = true)
                ...
                runCatching { PaymentRequests.sale(amount, currency, reference) }
                    .onSuccess { parameters ->
                        runCatching { TapToPhoneManager.mposUi.createTransactionIntent(parameters) }
                            .onSuccess { transactionId = ""; launchPayment(it) }
                            .onFailure { message = "Unable to prepare sale." }
            ...
            Button(enabled = !busy && transactionId.isNotBlank(), onClick = {
                ...
                }.onFailure { busy = false; message = "Unable to open transaction status." }
            }) { Text("Check status") }
                ...
                runCatching { PaymentRequests.refund(transactionId, refundAmount, currency) }
                    .onSuccess { parameters ->
                        runCatching { TapToPhoneManager.mposUi.createTransactionIntent(parameters) }
                            ...
                            .onFailure { message = "Unable to prepare refund." }
    ...
    pendingRefund?.let { intent ->
        AlertDialog(onDismissRequest = { pendingRefund = null },
            ...
            confirmButton = { TextButton(onClick = { pendingRefund = null; launchPayment(intent) }) { Text("Confirm refund") } },
